...
try:
    ...
    try:
        ...
        try:
            ...
            # Based on importlib example, but only needs to load_module since its
            # the whole package, not just a module
            ...
            raise ImportError(
                ...
                "AND could not find Ruamel YAML in the base conda environment"
                "through CONDA_EXE path. Environment not created!"
            )
...
if conda_path is None:
    raise RuntimeError(
        "Could not find a conda binary in "
        "CONDA_EXE variable or in executable search path"
    )
